...
**所有论文内容用英文撰写，但每段/每句上方添加中文注释（LaTeX注释格式）**。

这样作者可以：
1. **直接读中文**理解这段在讲什么
2. **快速判断**英文表达是否准确
3. **论文本身仍是纯英文**（注释不会出现在最终PDF中）

**写作格式示例**：

```latex
% 【中文注释】本文提出了一种自我验证机制，用于提升大语言模型在数学推理中的准确性。
% 具体来说，模型会在每个推理步骤后进行自检，如果发现错误则重新生成。
We propose a self-verification mechanism to improve the accuracy of large language models 
in mathematical reasoning. Specifically, the model performs self-checking after each 
reasoning step and regenerates if errors are detected.

% 【中文注释】现有方法（如标准思维链）经常会在中间步骤出现算术错误，导致最终答案错误。
Existing methods, such as standard chain-of-thought reasoning, often make arithmetic errors 
in intermediate steps, leading to incorrect final answers.

% 【中文注释】本文的核心创新是：不让模型一次性生成完整答案，而是每一步都验证后再继续。
The key innovation of this work is that instead of generating the complete answer at once, 
the model verifies each step before proceeding.
```

**注释规范**：
- 使用 `% 【中文注释】` 作为前缀
- 简要概括下方英文段落的核心意思
- 可以适当补充背景信息，帮助作者理解写作意图
- 对于公式、图表标题，同样需要中文注释

### .research/ 目录文件使用中文

`.research/` 目录下的所有记录文件（非论文正文）**全部使用中文撰写**：

| 文件 | 语言 | 内容 |
|------|------|------|
| `.research/IDEA.md` | 中文 | 研究想法、头脑风暴记录 |
| `.research/EXPERIMENT.md` | 中文 | 实验计划、结果记录 |
| `.research/REVIEW.md` | 中文 | 审稿意见 |
| `.research/REVIEW-HISTORY.md` | 中文 | 审稿历史追踪 |
| `.research/SOTA.md` | 中文 | SOTA论文分析 |

**原因**：
- 这些文件是**研究过程记录**，作者需要频繁阅读和修改
- 使用中文更直观，减少理解成本
- 论文正文（LaTeX）才是最终投稿的英文内容
